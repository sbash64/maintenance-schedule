...
add_to_schedule(
    schedule,
    Recurrence(what="clean vacuum filters", period=Period(months=2)),
    datetime.date.today(),
)
add_to_schedule(
    schedule,
    Recurrence(what="change car oil", period=Period(years=1)),
    datetime.date(2022, 4, 20),
)
add_to_schedule(
    schedule,
    Recurrence(what="backup computer", period=Period(months=1)),
